...
class TSC:
    ...
    def count_peak(self):
        ...
        # 计算跳跃次数
        parr = self.countPeakByJoin(joins, 9)  # 右肩
        parr2 = self.countPeakByJoin(joins, 10)  # 左肩
        # 提取跳跃运动特征
        feature = self.getSeq(df, parr)
    
        result = int((len(parr) + len(parr2)) / 2) - 1

        # 输出识别结果
        print("共做了", result , "次抛铅球。")
