...
meanF<-function(path, duration){
	...
	cq_dmj_Ts <- ts(cq_dmj[[2]],start=c(cq_dmj[1,1]%/%100%%100, cq_dmj[1,1]%%100),frequency=31)
	# 预测并处理预测结果
	...
	for (i in 1:duration) {
		LastMonth <- (LastDate%/%100%%100+(LastDate%%100 + i)%/%31)%%12
		AddYear <- (LastDate%/%100%%100+(LastDate%%100 + i)%/%31)%/%12
		foreDate[i] <- (((LastDate)%/%10000 + AddYear)*10000 + LastMonth*100 + (LastDate%%100 + i)%%31)}
