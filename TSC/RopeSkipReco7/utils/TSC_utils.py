...
def DTW(x, y):
    ...
    dist, cost, acc, path = dtw(x, y, keep_internals=True)
    return dist
...
# 读入并抽样参考序列
def readRef(path, rpath, rframe, pos):
    epath = rpath + path
    edf = pd.read_csv(epath)
    edf = sample_30(edf, rframe)
    joins = readJoin(edf)
    parr = countPeakByJoin(joins, pos)
...
def ifErr1(feature, st, e1, j0, j1):
    ...
    for seq in feature:
        c += 1
        st_dist = calErr1(seq, st, j0, j1)
        err_dist = calErr1(seq, e1, j0, j1)
