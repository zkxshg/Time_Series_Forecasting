...
meanF<-function(path, duration){
	...
	cq_dmj_Ts <- ts(cq_dmj[[2]],start=c(cq_dmj[1,1]%/%100, cq_dmj[1,1]%%100),frequency=12)
	# 预测并处理预测结果
	cq_dmjfit1 <- meanf(cq_dmj_Ts,h=duration)
	cq_df <- as.data.frame(cq_dmjfit1)
	foreDate <- ''
	LastDate <- as.integer(cq_dmj[[1]][length(cq_dmj[[1]])]) 
	for (i in 1:duration) {foreDate[i] <- ((LastDate)%/%100 + (LastDate%%100 + i)%/%12)*100 + (LastDate%%100 + i)%%12}
