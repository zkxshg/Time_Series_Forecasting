...
class TSC:
    ...
    def countPeakByJoin(self, df, pos):
        ...
        for i in range(1, length - 1):
            if (self.isPeak(left_df, i)):
                stat_list.append(left_df[i])
        
        # print(len(stat_list))
        peak_mean = np.mean(stat_list)
        peak_ssd = np.std(stat_list)
        # print("Mean = ", peak_mean, ", SSD = ", peak_ssd)
        # print("Range = [", peak_mean - peak_ssd, ",", peak_mean + peak_ssd, "]")
        ...
        for i in range(1, length - 1):
            if (self.isPeak(left_df, i)):
                if len(peak_arr) < 1:
                    ...
                    # if (left_df[i] - peak_mean) < (peak_ssd) :
                    if True:
                        peak_arr.append(i)
